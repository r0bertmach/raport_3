...
date: "`r Sys.Date()`" #zczytuje datę z systemu
#to jest szablon downcute
output:
  rmdformats::downcute:
    self_contained: true
    thumbnails: true
...
challenge_1 <- dane %>%
  ...
  slice_min(mdy(Release.Date), n=1)
  challenge_1
```
...
challenge_3 <- dane %>%
  ...
  mutate(
    Release.Date2 = as.Date(Release.Date, format = "%m/%d/%Y"),
    Netflix.Release.Date2 = as.Date(Netflix.Release.Date, format = "%m/%d/%Y"),
    days_difference = as.numeric(difftime(Netflix.Release.Date2,Release.Date2, units ="days")))

avg_time <- challenge_3 %>%  
  filter(year(Release.Date2) %in% c(2019,2020)) %>%
  summarize(average_difference = mean(days_difference, na.rm = TRUE))
  avg_time
```
...
mpt <- challenge_4 %>%
  mutate(Tags = strsplit(Tags, ",\\s*")) %>%
  unnest(Tags) %>%
  group_by(Tags) %>%
  summarise(liczba = n()) %>%
  slice_max(liczba, n=5)
  mpt
```
...
challenge_5 <- dane %>%
  ...
  mutate(year = year(mdy(Release.Date)),
         decade = 10 * (year %/% 10))

average_score <- challenge_5 %>%
  filter(!is.na(decade)) %>%
  group_by(decade) %>%
  summarize(avg_gem = mean(Hidden.Gem.Score, na.rm = TRUE),
            avg_IMDb = mean(IMDb.Score, na.rm = TRUE),
            avg_rotten = mean(Rotten.Tomatoes.Score, na.rm = TRUE),
            avg_meta = mean(Metacritic.Score, na.rm = TRUE)) %>%
  ungroup()
  average_score
```
